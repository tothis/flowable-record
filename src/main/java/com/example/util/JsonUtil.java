...
/**
 * @author 李磊
 * @time 2019/10/9 10:18:25
 * @description jackson json转换类
 */
public class JsonUtil {

    private static final ObjectMapper OBJECT_MAPPER = new ObjectMapper();
    private static final ObjectMapper OBJECT_MAPPER_LOWERCASE_WITH_UNDER_SCORES = new ObjectMapper();
    ...
    static {
        ...
        OBJECT_MAPPER_LOWERCASE_WITH_UNDER_SCORES.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
        // 序列化时添加下划线
        OBJECT_MAPPER_LOWERCASE_WITH_UNDER_SCORES.setPropertyNamingStrategy(new PropertyNamingStrategy.SnakeCaseStrategy());
    }
    ...
    public static <T> Set<T> jsonToSet(String json, Class<T> classType) {
        if (StringUtils.isNotBlank(json) && classType != null) {
            try {
                return OBJECT_MAPPER.readValue(json, OBJECT_MAPPER.getTypeFactory().constructCollectionType(Set.class, classType));
            } catch (IOException e) {
                System.out.println("json to list exception");
            }
        }
    ...
    public static String beanToJsonByLowerCase(Object object) {
        if (object != null) {
            try {
                return OBJECT_MAPPER_LOWERCASE_WITH_UNDER_SCORES.writeValueAsString(object);
            } catch (JsonProcessingException e) {
    ...
    public static <T> T jsonToBeanByLowerCase(String json, Class<T> classType) {
        if (StringUtils.isNotBlank(json)) {
            try {
                return OBJECT_MAPPER_LOWERCASE_WITH_UNDER_SCORES.readValue(json, classType);
            } catch (IOException e) {
